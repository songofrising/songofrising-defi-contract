...
// will be transferred to a governance smart contract once RubyToken is sufficiently
...
contract MasterGardener is Ownable, Authorizable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. RubyCoin to distribute per block.
        uint256 lastRewardBlock; // Last block number that RubyCoin distribution occurs.
        uint256 accGovTokenPerShare; // Accumulated RubyCoin per share, times 1e12. See below.
    }

    // The RubyToken token
    RubyToken public govToken;
    ...
    // RubyCoin created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus multiplier for early RubyCoin makers.
    ...
    // The block number when RubyCoin mining starts.
    ...
    // Update the given pool's RubyCoin allocation points. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        // Mint some new RubyCoin tokens for the farmer and store them in MasterGardener.
        ...
        )
    {
        ...
        if (GovernanceTokenCanMint < amount) {
            ...
            if(GovernanceTokenCanMint < (forDev.add(forFarmer).add(forLP).add(forCom).add(forFounders))){
                // If there aren't enough governance tokens left to mint before the cap,
                // just give all of the possible tokens left to the farmer.
                forDev = 0;
                forFarmer = amount;
    ...
    // View function to see pending RubyCoin on frontend.
    ...
    // Deposit LP tokens to MasterGardener for RubyCoin allocation.
